...
#figure(
  ...
  caption: [HGB feature-set comparison],
  kind: table,
)
...
The optimisation result is also instructive. Randomised search improved HGB cross-validated RMSE from 0.1443 to 0.1441 and test $R^2$ from 0.4166 to 0.4185 relative to the untuned HGB. The gain was genuine within the recorded run but small. Tuning therefore refined an already suitable model rather than transforming performance. This supports explicit optimisation while cautioning against presenting search as inherently valuable irrespective of its measured benefit.

== Predictive importance and anomalies

Figure 4 shows that #raw("Traditional_Study_Hours") had the largest permutation importance (0.0328 increase in RMSE-based loss when shuffled), followed by #raw("Primary_Use_Case") (0.0267), #raw("Weekly_GenAI_Hours") (0.0174), #raw("Year_of_Study") (0.0136), #raw("Prompt_Engineering_Skill") (0.0122) and #raw("Pre_Semester_GPA") (0.0108). Institutional policy and paid subscription made small contributions, while tool diversity, perceived dependency, exam anxiety and major category were near zero in this fitted model.

#figure(
  image("../../assets/fig04_permutation_importance.png", width: 100%),
  caption: [Test-set permutation importance for the selected tuned HGB model. Values show the mean deterioration in RMSE-based score after shuffling each source feature across five repetitions.],
)
...
The final $R^2$ of 0.4185 also means that most test-set variation remained unaccounted for. Missing influences could include course difficulty, assessment format, instructor practice, prior ability, socioeconomic context and measurement error. Residual compression at extreme outcomes reinforces the need for caution when applying average-performing models to unusual students.

== Recommendations

First, the tuned HGB should be retained as the assignment's predictive benchmark because it led both cross-validation and test evaluation. It should not be deployed as an intervention rule or used to label individual students as likely to benefit from AI. At most, its outputs could support hypothesis generation after independent validation.
