...
Every candidate used the same train-test indices, five cross-validation folds and preprocessing architecture. This controlled comparison isolates differences in the estimators rather than allowing each model to benefit from a different data treatment. Numeric columns were median-imputed and standardised. Categorical and Boolean columns were most-frequent-imputed and one-hot encoded. The dense encoded matrix was then supplied to the estimator. Because the preprocessor and estimator formed one #raw("Pipeline"), fitting within a cross-validation fold could not learn category levels, medians, modes or scale parameters from its validation partition.

The mean baseline was essential even though it was not a learning model. It predicted the development-set mean GPA change for every record and established the minimum performance that a useful regression model should exceed. Its expected $R^2$ is around zero on unseen data; a negative value means that even the fixed mean would perform better.

== Candidate regressors

Ordinary least-squares Linear Regression provided an interpretable additive benchmark. It assumes that the encoded predictors contribute through linear combinations and therefore cannot directly represent thresholds or interactions. Ridge Regression used the same functional form with an L2 penalty and #raw("alpha=1.0"). It tests whether shrinking correlated or weak coefficients improves generalisation.

Random Forest represented bagged nonlinear trees. The implementation used 180 trees, a minimum of three samples per leaf and 80% of available features at each split (#raw("max_features=0.8")). Setting #raw("n_jobs=1") within the estimator avoided nested model-level parallelism, while the cross-validation operation could run folds in parallel. A fixed random state made bootstrap sampling and feature selection reproducible. Tree ensembles were suitable because the exploratory patterns suggested possible thresholds and interactions, and comparable educational-data-mining studies have found tree-based methods competitive with linear and other classifiers (Yağcı, 2022). That prior classification result was methodological context, not a numerical benchmark for this regression.

Histogram Gradient Boosting built trees sequentially, with each iteration reducing residual error from the preceding ensemble. Histogram binning makes the method efficient for 50,000 rows. The untuned comparison used 180 boosting iterations, learning rate 0.07 and 31 maximum leaf nodes. Its capacity to learn non-additive structure was relevant to the non-monotonic AI-hours pattern and the ceiling-shaped relation between previous GPA and GPA change.

#figure(
  {
    compact-table(size: 9pt)[
      #table(
        columns: (1.05fr, 1.55fr, 1.35fr),
        align: left + top,
        inset: (x: 4pt, y: 3pt),
        ...
        [Mean baseline],
        [#raw("strategy=\"mean\"")],
        [Minimum predictive reference],
        [Linear Regression],
        [scikit-learn defaults],
        [Unregularised additive benchmark],
        [Ridge Regression],
        [#raw("alpha=1.0")],
        [Regularised additive benchmark],
        [Random Forest],
        [180 trees; minimum leaf 3; 80% features; seed 42],
        [Bagged nonlinear ensemble],
        [HGB],
        [180 iterations; rate 0.07; 31 leaves; seed 42],
        [Boosted nonlinear ensemble],
      )
    ]
  },
  caption: [Candidate model specification],
  kind: table,
)
...
HGB achieved the lowest mean cross-validated RMSE in the untuned comparison and was therefore the only family passed to optimisation. Restricting tuning to the development data preserved the test set for confirmation. A randomised search evaluated ten sampled combinations from the space in Table 6. RMSE was the search objective because it expresses error in GPA points while assigning greater cost to large mistakes.

#figure(
  {
    compact-table(size: 9pt)[
      #table(
        columns: (1.25fr, 1.7fr, 0.75fr),
        align: left + top,
        inset: (x: 4pt, y: 3pt),
        stroke: 0.45pt + rule-grey,
        fill: (x, y) => if y == 0 { pale-grey },
  ...
  caption: [HGB randomised-search space and selected configuration],
  kind: table,
)
